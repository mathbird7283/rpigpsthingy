...
def optimize1(s1,s2,e1,e2,restricted=[],depth=0,path=[],lookedat={},printe=True):
    ...
    for item in news:
        ...
        if ((not lookedat.has_key((item[0],item[1])) and not lookedat.has_key((item[1],item[0]))) or (lookedat.has_key((item[0],item[1])) and lookedat[(item[0],item[1])]>depth) or (lookedat.has_key((item[1],item[0])) and lookedat[(item[1],item[0])]>depth)): # tests if this is the best (currently) path
